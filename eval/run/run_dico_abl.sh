...
ACCELERATION_LOW_THRESHOLDS=(0.55)
ACCELERATION_FACTOR=1
R_GATES=(0.8)
MOPUP_MARGIN_THRESHOLDS=(3.0)
MOPUP_SPEED=1
...
for TG_ALPHA in "${TG_ALPHAS[@]}"
do
  for TG_BETA in "${TG_BETAS[@]}"
  do
    for EXPLORATION_N in "${EXPLORATION_NS[@]}"
    do
      for EXPLORATION_THRESHOLD in "${EXPLORATION_THRESHOLDS[@]}"
      do
        for ACCELERATION_LOW_THRESHOLD in "${ACCELERATION_LOW_THRESHOLDS[@]}"
        do
          for R_GATE in "${R_GATES[@]}"
          do
            for MOPUP_MARGIN_THRESHOLD in "${MOPUP_MARGIN_THRESHOLDS[@]}"
            do
              ...
              if [ "$RECORD_TIMING_BREAKDOWN" -eq 1 ]; then
                HYPERPARAM_DIR="${HYPERPARAM_DIR}_timingbreakdown"
              fi
